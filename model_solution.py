...
class CausalAttention(nn.Module):
    ...
    def forward(
        ...
        # Reshape to split heads: (batch, seq_len, n_heads, d_attention)
        q = rearrange(q, 'b s (h d) -> b h s d', h=self.n_heads)
        k = rearrange(k, 'b s (h d) -> b h s d', h=self.n_heads)
        v = rearrange(v, 'b s (h d) -> b h s d', h=self.n_heads)

        # compute h = softmax(qk.t/sqrt(d))v
        ...
        h = rearrange(h, 'b h s d -> b s (h d)', h=self.n_heads)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        a_pre = self.attention(self.pre_layer_norm(x)) + x
        ff_pre = self.mlp(self.post_layer_norm(a_pre)) + a_pre
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        pos = torch.arange(0, seq_len).expand(batch_size, seq_len).to(device=x.device)
        output = self.embeddings(x) + self.position_embeddings(pos)
        
        for dec_block in self.backbone:
    ...
    def generate(
        ...
        # we'll generate the tokens one by one and we'll feed again into forward
        for _ in range(num_new_tokens):
            x = torch.cat(
                (
                    x,
                    self.forward(x)[:, -1, :].argmax(dim=-1, keepdim=True)
                ), dim=-1)
    ...
    def get_loss_on_batch(
        ...
        output = self.forward(input_ids)[:, :-1, :].reshape(-1, self.config.vocab_size)
        loss = F.cross_entropy(output, input_ids[:, 1:].reshape(-1))
